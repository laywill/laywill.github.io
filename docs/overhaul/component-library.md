...
- Responsive behaviour — the 360px floor, vertical monitors — is not covered.
  That is what `scripts/viewport-check.mjs` is for; see below.

## Viewport checking

`scripts/viewport-check.mjs` drives a real headless Chrome over the DevTools
Protocol and reports the document's scroll width against its client width for
every route at every documented breakpoint, plus the two shapes the acceptance
criteria name: a 1080×1920 vertical monitor, and a 1024×600 short-and-wide
window. It exits non-zero on any horizontal scroll.

It needs a browser and a running preview server, so it is **not** wired into
CI — it is a hand-run check before a layout change lands:
...
This exists because reading the CSS carefully is demonstrably not sufficient.
Three separate horizontal-scroll defects shipped into this branch and were
invisible to `astro check`, to `astro build` and to the axe run, because none
of those lays out a page:

| Defect                                                                                        | Effect                                      |
| --------------------------------------------------------------------------------------------- | ------------------------------------------- |
| `ResponsiveImage` set no `max-width`, so `astro:assets`' intrinsic dimensions rendered at 1:1 | Document 2000px wide at every viewport      |
| The `box-sizing` reset sat in a *scoped* `<style>`, so it reached no component                | `.content-col` 32px past the viewport       |
| A 1px `.visually-hidden` span, absolutely positioned with no positioned ancestor              | Escaped the nav's scroll container to 596px |

**Result at the time of writing** — both routes, zero overflow:
...
| 900 (`md`)          | ok  | ok             |
| 1200 (`lg`)         | ok  | ok             |
| 1080×1920 vertical  | ok  | ok             |
| 1024×600 short-wide | ok  | ok             |

What it still does not cover: keyboard operation, reading order, focus
behaviour, and whether the result actually looks right. Those remain a human
job.
